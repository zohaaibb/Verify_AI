    ...
    def decorator(func):
        ...
        def wrapper(self, input_type, content):
            if input_type not in allowed_types:
                logger.warning(f"⚠️  {func.__name__} not applicable for '{input_type}'")
                return {
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"⚠️  {func.__name__} attempt {attempt + 1}/{max_retries} "
...
class AnalysisOrchestrator:
    ...
    def __init__(self):
        ...
        self.sandbox_orchestrator = get_sandbox_orchestrator(self.virustotal)

        try:
            self.url_ml_detector.load()
        except Exception as e:
            logger.warning(f"⚠️ URL ML detector not loaded: {e} — will use heuristic fallback")
    ...
    def _init_module_registry(self):
        ...
        self.registry.register('sandbox',    self.run_sandbox_analysis,
                               summarizer=self._summarize_sandbox,
                               input_types=['url', 'file', 'image'])
        ...
        self.registry.register('reverse',    self.run_reverse_analysis,
    ...
    def run_url_pipeline(self, url: str, selected_modules: List[str]) -> Dict:
        ...
        phase_start = time.time()
        if 'reverse' in applicable or 'sandbox' in applicable:
            try:
    ...
    def _run_domain_pipeline(self, url: str, selected_modules: List[str],
        ...
        """
        logger.info(f"🏷️   Domain/Brand pipeline for: {url}")
        ...
        results = {
            ...
            'pipeline_stats':     {'phases': {}},
            'notice': (
                "ℹ️  This is a domain/brand credibility check. "
        ...
        if skipped:
            ...
            logger.info(f"⏭️   Skipping (not applicable to domain URLs): {skipped}")
    ...
    def _generate_domain_summary(self, module_results: Dict, url_type_info: Dict) -> Dict:
        ...
        if score >= 70:
            ...
            summary['recommendations'].append(
                "⚠️  CAUTION: This domain has some suspicious indicators. "
            ...
            summary['recommendations'].append(
                "ℹ️  NEUTRAL: No major threats detected, but limited credibility data available."
            )
    ...
    def run_image_pipeline(self, image_path: str, selected_modules: List[str],
        ...
        logger.info(f"🖼️  Image pipeline – modules: {applicable}")
        ...
        if not extracted_text:
            try:
                ...
                logger.warning(f"⚠️ Fallback OCR failed: {e}")
        ...
        # ✏️ USER-EDITED TEXT OVERRIDE (agar frontend ne bheja ho)
        if user_edited_text:
            user_edited_text = user_edited_text.strip()
            if user_edited_text:
                extracted_text = user_edited_text
                logger.info(f"✏️ Using user-edited text ({len(extracted_text.split())} words): {extracted_text[:200]}")
        ...
        if extracted_text and len(extracted_text.split()) >= 3:
            ...
            logger.warning(f"⚠️ OCR text too short, skipping AI/OSINT")
    ...
    @timed_execution
    def run_sandbox_analysis(self, input_type: str, content: str) -> Dict:
        if input_type == 'file':
    ...
    def run_reverse_analysis(self, input_type: str, content: str) -> Dict:
        if input_type == 'url':
    ...
    def _generate_combined_summary(self, module_results: Dict, input_type: Optional[str] = None) -> Dict:
        ...
        if final <= 15:
            ...
            summary['recommendations'].append("ℹ️ LOW THREAT: Some indicators present. Exercise normal caution.")
        elif final <= 70:
            summary['verdict'] = 'MEDIUM THREAT'
            summary['recommendations'].append("⚠️ MEDIUM THREAT: Multiple suspicious indicators detected. Verify through trusted sources before sharing.")
        else:
